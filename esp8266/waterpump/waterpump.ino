...
void handleRelay() {
  ...
  digitalWrite(relayPin,
               !digitalRead(relayPin));  // Change the state of the Relay
  server.sendHeader("Location", "/");
  server.send(303);
}
...
BLYNK_APP_CONNECTED() { Serial.println("[Blynk] App Connected!"); }

BLYNK_APP_DISCONNECTED() { Serial.println("[Blynk] App Disconnected!"); }
